      ...
      new ArrayList<>(actionBarList.getValue()).stream().max(Comparator.comparingInt(ActionBar::getPriority)).ifPresent(actionBar -> {
        switch(actionBar.getActionBarType()) {
          ...
          case COOLDOWN:
            ...
            break;
          case DISPLAY:
            VersionUtils.sendActionBar(player, actionBar.getMessage().arena(plugin.getArenaRegistry().getArena(player)).player(player).integer((actionBar.getTicks() - actionBar.getExecutedTicks()) / 20).build());
            break;
          case PROGRESS:
